...
const TempSensor4200BFaceplate3E = () => {
  ...
  const [stringFields, setStringFields] = useState({
    TAGNAME: '1540-TI-4200B',
    DESC: 'Furnace Temp Out B',
    UNIT: ''
  });
  ...
  useEffect(() => {
    ...
    // If no saved config, use furnace temperature-appropriate defaults
    if (!savedConfig.TAGNAME || savedConfig.TAGNAME === defaultSecondaryConfig.TAGNAME) {
      ...
      setStringFields({
        TAGNAME: savedConfig.TAGNAME || '1540-TI-4200B',
        DESC: savedConfig.DESC || 'Furnace Temp Out B',
  ...
  const handleApply = useCallback(() => {
    ...
    // Sync live values to sync context - use TYPICAL_PV if configured
    // Also set SP = TYPICAL_PV so the simulation maintains the PV value (error = 0)
    ...
    // Sync alarm limits to context for automatic alarm state updates
    updateAlarmLimits({
...
export default TempSensor4200BFaceplate3E;