...
const TEMP_SENSOR_4841_PID_CONFIG_KEY = 'temp_sensor_4841_pid_config';
...
const TempSensor4841Faceplate3A = () => {
  const activeControllerId = '1540-TI-4841';
  
  const { toast } = useToast();
  const { state } = useControllerSync(activeControllerId);
  const [config, setConfig] = useState<PIDHxBypassConfig>({
    ...defaultPIDHxBypassConfig,
    loop_tag: '1540-TI-4841',
    service_desc: 'Pass 2 Catalyst Out',
    eng_units: 'F',
  });
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4841_PID_CONFIG_KEY);
    if (saved) {
      try {
  ...
  const saveConfig = useCallback(() => {
    localStorage.setItem(TEMP_SENSOR_4841_PID_CONFIG_KEY, JSON.stringify(config));
      ...
      <div className="container mx-auto py-6 px-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            <Link
              to="/settings/controller-outputs/faceplates/temp-sensor/1540-TI-4841"
              className="inline-flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition-colors"
            >
              <ArrowLeft size={18} />
              <span>Back to 1540-TI-4841 Temperature Sensor</span>
            </Link>
        ...
        <div className="grid grid-cols-4 gap-4 mt-6">
          ...
          <div className="bg-muted/30 rounded-lg p-3 border border-border/50">
            ...
            <p className={`text-sm font-mono ${
              config.mode === 'AUTO' ? 'text-green-400' : 
              config.mode === 'MAN' ? 'text-orange-400' : 'text-cyan-400'
            }`}>{config.mode}</p>
          </div>
          <div className="bg-muted/30 rounded-lg p-3 border border-border/50">
            <h3 className="text-xs font-medium text-muted-foreground mb-1">PV</h3>
            <p className="text-sm font-mono text-amber-400">{state.syncedPV.toFixed(1)} {config.eng_units}</p>
          </div>
          <div className="bg-muted/30 rounded-lg p-3 border border-border/50">
            <h3 className="text-xs font-medium text-muted-foreground mb-1">SP</h3>
            <p className="text-sm font-mono text-white">{state.syncedSP.toFixed(1)} {config.eng_units}</p>
          </div>
        </div>
      </div>
    </div>
...
export default TempSensor4841Faceplate3A;
