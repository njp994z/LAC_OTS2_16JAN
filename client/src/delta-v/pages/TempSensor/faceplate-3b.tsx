...
const TempSensorFaceplate3B = () => {
  const {temperatureSensorId: activeControllerId} = useParams();

  const TEMP_SENSOR_TIMESERIES_KEY = `temp_sensor_${activeControllerId}_timeseries`;
  const TEMP_SENSOR_PID_CONFIG_KEY = `temp_sensor_${activeControllerId}_pid_config`;
  ...
  const [config, setConfig] = useState({
    ...
    loop_tag: activeControllerId!,
    service_desc: 'DT Gas Out Temperature',
    eng_units: 'C',
  });

  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_PID_CONFIG_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_TIMESERIES_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (logData.length > 0) {
      localStorage.setItem(TEMP_SENSOR_TIMESERIES_KEY, JSON.stringify(logData.slice(-maxRows)));
    }
  }, [logData, maxRows]);
  ...
  const handleClearLog = () => {
    setLogData([]);
    localStorage.removeItem(TEMP_SENSOR_TIMESERIES_KEY);
  };
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `temp_sensor_5821_timeseries_${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.csv`;
    a.click();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 deltav-grid-pattern">
      <div className="container mx-auto py-6 px-4">
        <Link
          to={`/settings/controller-outputs/faceplates/temperature-sensor/${activeControllerId}`}
          className="inline-flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition-colors mb-4"
        >
          <ArrowLeft size={18} />
          <span>Back to {activeControllerId} Temperature Sensor</span>
        </Link>
...
export default TempSensorFaceplate3B;
