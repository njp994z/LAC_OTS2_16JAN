...
const TempSensorMain = () => {
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({
    ...
    instrumentTag: savedConfig.TAGNAME || activeControllerId!,
    description: savedConfig.DESC || 'Furnace Temp Out A',
  ...
  useEffect(() => {
    const syncConfigFromContext = () => {
      ...
      setControllerData(prev => ({
        ...prev,
        instrumentTag: latestConfig.TAGNAME || activeControllerId!,
        description: latestConfig.DESC || 'Furnace Temp Out A',
        pvUnits: sanitizedEU,
  ...
  // Force sync PV and SP to TYPICAL_PV on mount and when config changes
  // This bypasses the "initialized" check and ensures the sensor displays the configured value
  useEffect(() => {
  ...
  useEffect(() => {
    (window as any).updateTempSensor4200AControllerData = (newData: Partial<ControllerData>) => {
      setControllerData(prev => ({ ...prev, ...newData }));
    };
    (window as any).updateTempSensor4200ASecondaryData = (newData: Partial<SecondaryControllerData>) => {
      setSecondaryData(prev => ({ ...prev, ...newData }));
    };
    
    return () => {
      delete (window as any).updateTempSensor4200AControllerData;
      delete (window as any).updateTempSensor4200ASecondaryData;
    };
  }, []);
