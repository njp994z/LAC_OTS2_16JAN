...
const TEMP_SENSOR_5821_TIMESERIES_KEY = 'temp_sensor_5821_timeseries';
const TEMP_SENSOR_5821_PID_CONFIG_KEY = 'temp_sensor_5821_pid_config';

const TempSensor5821Faceplate3B = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_5821_TIMESERIES_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (logData.length > 0) {
      localStorage.setItem(TEMP_SENSOR_5821_TIMESERIES_KEY, JSON.stringify(logData.slice(-maxRows)));
    }
  }, [logData, maxRows]);
  ...
  const handleClearLog = () => {
    setLogData([]);
    localStorage.removeItem(TEMP_SENSOR_5821_TIMESERIES_KEY);
  };
...
export default TempSensor5821Faceplate3B;
