...
const TEMP_SENSOR_4820_PID_CONFIG_KEY = 'temp_sensor_4820_pid_config';
...
const TempSensor4820Faceplate3A = () => {
  const activeControllerId = '1540-TI-4820';
  
  const { toast } = useToast();
  const { state } = useControllerSync(activeControllerId);
  const [config, setConfig] = useState<PIDHxBypassConfig>({
    ...defaultPIDHxBypassConfig,
    loop_tag: '1540-TI-4820',
    service_desc: 'Pass 1 Inlet Duct',
    eng_units: 'F',
  });
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4820_PID_CONFIG_KEY);
    if (saved) {
      try {
  ...
  const saveConfig = useCallback(() => {
    localStorage.setItem(TEMP_SENSOR_4820_PID_CONFIG_KEY, JSON.stringify(config));
      ...
      <div className="container mx-auto py-6 px-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            <Link
              to="/settings/controller-outputs/faceplates/temp-sensor/1540-TI-4820"
              className="inline-flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition-colors"
            >
              <ArrowLeft size={18} />
              <span>Back to 1540-TI-4820 Temperature Sensor</span>
            </Link>
...
export default TempSensor4820Faceplate3A;
