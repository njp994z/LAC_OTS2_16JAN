...
const HAND_CONTROLLER_4283_TIMESERIES_KEY = 'hand_controller_4283_timeseries';
const HAND_CONTROLLER_4283_PID_CONFIG_KEY = 'hand_controller_4283_pid_config';

const HandController4283Faceplate3B = () => {
  ...
  const [config, setConfig] = useState({
    ...
    loop_tag: '1540-H-4283',
    service_desc: 'WHB Outlet dP Hand Controller',
  });

  useEffect(() => {
    const saved = localStorage.getItem(HAND_CONTROLLER_4283_PID_CONFIG_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(HAND_CONTROLLER_4283_TIMESERIES_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (logData.length > 0) {
      localStorage.setItem(HAND_CONTROLLER_4283_TIMESERIES_KEY, JSON.stringify(logData.slice(-maxRows)));
    }
  }, [logData, maxRows]);
  ...
  const handleClearLog = () => {
    setLogData([]);
    localStorage.removeItem(HAND_CONTROLLER_4283_TIMESERIES_KEY);
  };
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `hand_controller_4283_timeseries_${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.csv`;
    a.click();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 deltav-grid-pattern">
      <div className="container mx-auto py-6 px-4">
        ...
        >
          <ArrowLeft size={18} />
          <span>Back to 1540-H-4283 WHB Outlet dP Hand Controller</span>
        </Link>
...
export default HandController4283Faceplate3B;
