...
const WHBOutletHandControllerFaceplateMain = () => {
  const activeControllerId = '1540-H-4283';
  const { state: syncState, updateSyncedPV, updateSyncedSP, updateSyncedOUT, updateSyncedMode, initializeController, updateAlarmLimits } = useControllerSync(activeControllerId);
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({
    ...
    instrumentTag: savedConfig.TAGNAME || '1540-H-4283',
    description: savedConfig.DESC || 'WHB Outlet dP Hand Controller',
  ...
  useEffect(() => {
    ...
    // Use explicit finite check instead of ?? (which doesn't catch NaN)
    const initialPV = Number.isFinite(latestConfig.PV_INIT_VAL) ? latestConfig.PV_INIT_VAL : 50;
    const initialSP = Number.isFinite(latestConfig.SP_LIM_HI) ? latestConfig.SP_LIM_HI : 50;
    const pvScaleLo = Number.isFinite(latestConfig.PV_SCALE_LO) ? latestConfig.PV_SCALE_LO : 0;
    const pvScaleHi = Number.isFinite(latestConfig.PV_SCALE_HI) ? latestConfig.PV_SCALE_HI : 100;
    ...
    // Force update PV to ensure it's not NaN (fixes corrupted localStorage)
    updateSyncedPV(initialPV);
    
    updateAlarmLimits({
      LL: Number.isFinite(latestConfig.ALM_LL_LIM) ? latestConfig.ALM_LL_LIM : 0,
      L: Number.isFinite(latestConfig.ALM_L_LIM) ? latestConfig.ALM_L_LIM : 0,
      H: Number.isFinite(latestConfig.ALM_H_LIM) ? latestConfig.ALM_H_LIM : 0,
      HH: Number.isFinite(latestConfig.ALM_HH_LIM) ? latestConfig.ALM_HH_LIM : 0,
    });
  }, []);
  ...
  useEffect(() => {
    const syncConfigFromContext = () => {
      ...
      setControllerData(prev => ({
        ...prev,
        instrumentTag: latestConfig.TAGNAME || '1540-H-4283',
        description: latestConfig.DESC || 'WHB Outlet dP Hand Controller',
        pvUnits: latestConfig.EU || defaultControllerData.pvUnits,
        pvRangeMin: latestConfig.PV_SCALE_LO ?? defaultControllerData.pvRangeMin,
        pvRangeMax: latestConfig.PV_SCALE_HI ?? defaultControllerData.pvRangeMax,
      }));
    };
      ...
      <div className="relative max-w-4xl mx-auto">
        ...
        <header className="mb-10 text-center">
          <h1 className={cn(
            ...
            1540-H-4283 WHB Outlet dP Hand Controller Faceplate
          </h1>
...
export default WHBOutletHandControllerFaceplateMain;
