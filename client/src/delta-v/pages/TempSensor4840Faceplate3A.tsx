...
const TEMP_SENSOR_4840_PID_CONFIG_KEY = 'temp_sensor_4840_pid_config';
...
const TempSensor4840Faceplate3A = () => {
  const activeControllerId = '1540-TI-4840';
  
  const { toast } = useToast();
  const { state } = useControllerSync(activeControllerId);
  const [config, setConfig] = useState<PIDHxBypassConfig>({
    ...defaultPIDHxBypassConfig,
    loop_tag: '1540-TI-4840',
    service_desc: 'Pass 2 Catalyst In',
    eng_units: 'F',
  });
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4840_PID_CONFIG_KEY);
    if (saved) {
      try {
  ...
  const saveConfig = useCallback(() => {
    localStorage.setItem(TEMP_SENSOR_4840_PID_CONFIG_KEY, JSON.stringify(config));
      ...
      <div className="container mx-auto py-6 px-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            <Link
              to="/settings/controller-outputs/faceplates/temp-sensor/1540-TI-4840"
              className="inline-flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition-colors"
            >
              <ArrowLeft size={18} />
              <span>Back to 1540-TI-4840 Temperature Sensor</span>
            </Link>
...
export default TempSensor4840Faceplate3A;
