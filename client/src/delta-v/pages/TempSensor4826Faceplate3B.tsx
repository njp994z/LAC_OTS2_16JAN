...
const TEMP_SENSOR_4826_TIMESERIES_KEY = 'temp_sensor_4826_timeseries';
const TEMP_SENSOR_4826_PID_CONFIG_KEY = 'temp_sensor_4826_pid_config';

const TempSensor4826Faceplate3B = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4826_TIMESERIES_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (logData.length > 0) {
      localStorage.setItem(TEMP_SENSOR_4826_TIMESERIES_KEY, JSON.stringify(logData.slice(-maxRows)));
    }
  }, [logData, maxRows]);
  ...
  const handleClearLog = () => {
    setLogData([]);
    localStorage.removeItem(TEMP_SENSOR_4826_TIMESERIES_KEY);
  };
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `temp_sensor_4826_timeseries_${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.csv`;
    a.click();
...
export default TempSensor4826Faceplate3B;
