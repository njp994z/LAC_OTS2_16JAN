...
const TempSensor4825Main = () => {
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({
    ...
    instrumentTag: savedConfig.TAGNAME || '1540-TI-4825',
    description: savedConfig.DESC || 'Pass 1 Catalyst In',
  ...
  useEffect(() => {
    const syncConfigFromContext = () => {
      ...
      setControllerData(prev => ({
        ...prev,
        instrumentTag: latestConfig.TAGNAME || '1540-TI-4825',
        description: latestConfig.DESC || 'Pass 1 Catalyst In',
        pvUnits: sanitizedEU,
  ...
  useEffect(() => {
    (window as any).updateTempSensor4825ControllerData = (newData: Partial<ControllerData>) => {
      setControllerData(prev => ({ ...prev, ...newData }));
    };
    (window as any).updateTempSensor4825SecondaryData = (newData: Partial<SecondaryControllerData>) => {
      setSecondaryData(prev => ({ ...prev, ...newData }));
    };
    
    return () => {
      delete (window as any).updateTempSensor4825ControllerData;
      delete (window as any).updateTempSensor4825SecondaryData;
    };
  }, []);
