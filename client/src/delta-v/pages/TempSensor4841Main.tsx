...
const TempSensor4841Main = () => {
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({
    ...
    instrumentTag: savedConfig.TAGNAME || '1540-TI-4841',
    description: savedConfig.DESC || 'Pass 2 Catalyst Out',
  ...
  useEffect(() => {
    const syncConfigFromContext = () => {
      ...
      setControllerData(prev => ({
        ...prev,
        instrumentTag: latestConfig.TAGNAME || '1540-TI-4841',
        description: latestConfig.DESC || 'Pass 2 Catalyst Out',
        pvUnits: sanitizedEU,
  ...
  useEffect(() => {
    (window as any).updateTempSensor4841ControllerData = (newData: Partial<ControllerData>) => {
      setControllerData(prev => ({ ...prev, ...newData }));
    };
    (window as any).updateTempSensor4841SecondaryData = (newData: Partial<SecondaryControllerData>) => {
      setSecondaryData(prev => ({ ...prev, ...newData }));
    };
    
    return () => {
      delete (window as any).updateTempSensor4841ControllerData;
      delete (window as any).updateTempSensor4841SecondaryData;
    };
  }, []);
...
export default TempSensor4841Main;
