...
const HAND_CONTROLLER_4030_TIMESERIES_KEY = 'hand_controller_4030_timeseries';
const HAND_CONTROLLER_4030_PID_CONFIG_KEY = 'hand_controller_4030_pid_config';

const HandController4030Faceplate3B = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(HAND_CONTROLLER_4030_TIMESERIES_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (logData.length > 0) {
      localStorage.setItem(HAND_CONTROLLER_4030_TIMESERIES_KEY, JSON.stringify(logData.slice(-maxRows)));
    }
  }, [logData, maxRows]);
  ...
  const handleClearLog = () => {
    setLogData([]);
    localStorage.removeItem(HAND_CONTROLLER_4030_TIMESERIES_KEY);
  };
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `hand_controller_4030_timeseries_${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.csv`;
    a.click();
...
export default HandController4030Faceplate3B;
