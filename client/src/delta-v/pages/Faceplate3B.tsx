...
const Faceplate3B = () => {
  ...
  const [sampleRate, setSampleRate] = useState(1000); // ms
  ...
  // Load saved config for nominal values
  const [config, setConfig] = useState(defaultPIDHxBypassConfig);
  ...
  useEffect(() => {
    const saved = localStorage.getItem(LOOP_TIMESERIES_STORAGE_KEY);
    if (saved) {
      try {
  ...
  // Save log data when it changes
  useEffect(() => {
    if (logData.length > 0) {
      localStorage.setItem(LOOP_TIMESERIES_STORAGE_KEY, JSON.stringify(logData.slice(-maxRows)));
    }
  }, [logData, maxRows]);
  ...
  // Keep capture function ref updated
  useEffect(() => {
    captureRef.current = captureDataPoint;
  }, [captureDataPoint]);

  // Logging interval - only depends on isLogging and sampleRate
  useEffect(() => {
    if (isLogging) {
  ...
  const handleClearLog = () => {
    setLogData([]);
    localStorage.removeItem(LOOP_TIMESERIES_STORAGE_KEY);
  };
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `loop_timeseries_${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.csv`;
    a.click();
      ...
      <div className="container mx-auto py-6 px-4">
        ...
        <div className="bg-card/80 backdrop-blur border border-border rounded-lg p-6 max-w-6xl mx-auto">
              ...
              <Table>
                <TableHeader className="sticky top-0 bg-slate-800 z-10">
                  <TableRow className="hover:bg-transparent">
                    ...
                    <TableHead className="text-xs font-semibold text-muted-foreground w-14 text-right">Tin</TableHead>
                    <TableHead className="text-xs font-semibold text-muted-foreground w-14 text-right">Thx</TableHead>
                    <TableHead className="text-xs font-semibold text-muted-foreground w-14 text-right">Tout</TableHead>
                    ...
                    <TableHead className="text-xs font-semibold text-purple-400 w-16 text-right">x</TableHead>
                  ...
                  {logData.length === 0 ? (
                    ...
                    logData.map((row, idx) => (
                      <TableRow key={idx} className={`${getRowClassName(row)} hover:bg-muted/30`}>
                        ...
                        <TableCell className="font-mono text-xs text-muted-foreground text-right py-1.5">
                          {row.Tin.toFixed(1)}
                        </TableCell>
                        <TableCell className="font-mono text-xs text-muted-foreground text-right py-1.5">
                          {row.Thx.toFixed(1)}
                        ...
                        <TableCell className="font-mono text-xs text-purple-400 text-right py-1.5">
                          {row.x.toFixed(3)}
                        </TableCell>
          ...
          <div className="flex flex-wrap gap-4 mt-3 text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-purple-500/20 border border-purple-500/30" /> BYPASS mode
            </span>
          </div>
        </div>
      </div>
