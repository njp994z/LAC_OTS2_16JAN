...
const TEMP_SENSOR_4826_PID_CONFIG_KEY = 'temp_sensor_4826_pid_config';
...
const TempSensor4826Faceplate3A = () => {
  ...
  const [config, setConfig] = useState<PIDHxBypassConfig>({
    ...defaultPIDHxBypassConfig,
    loop_tag: '1540-TI-4826',
    service_desc: 'Pass 1 Catalyst Temp. Top B',
    eng_units: 'F',
  });
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4826_PID_CONFIG_KEY);
    if (saved) {
      try {
  ...
  const saveConfig = useCallback(() => {
    localStorage.setItem(TEMP_SENSOR_4826_PID_CONFIG_KEY, JSON.stringify(config));
...
export default TempSensor4826Faceplate3A;
