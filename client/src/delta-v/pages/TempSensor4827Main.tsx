...
const TempSensor4827Main = () => {
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({ ...defaultControllerData, instrumentTag: '1540-TI-4827', description: 'Pass 1 Catalyst Out', pvUnits: 'F', pvRangeMin: 0, pvRangeMax: 2000 });
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      ...
      <div className="relative max-w-4xl mx-auto">
        <Link href="/settings/controller-outputs/faceplates/temp-sensor/1540-TI-4827" className="inline-flex items-center gap-2 px-4 py-2 text-sm text-muted-foreground hover:text-foreground mb-6"><ArrowLeft size={18} />Back</Link>
        <header className="mb-8 text-center"><h1 className="text-3xl font-bold mb-3 bg-gradient-to-r from-primary via-cyan-400 to-primary bg-clip-text text-transparent">1540-TI-4827 Pass 1 Catalyst Out</h1></header>
        <div className="flex flex-wrap justify-center gap-8">
          ...
          <div className="flex flex-col items-center gap-2"><span className="text-xs text-muted-foreground uppercase">Secondary</span><TempSensorSecondaryFaceplate data={secondaryData} config={config} sensorId="1540-TI-4827" /></div>
        </div>
      </div>
    </div>
  );
