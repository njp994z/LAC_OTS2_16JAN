...
const positionSensors = [
  { tag: '1540-ZI-4282', description: 'Jug Valve Positioner' },
];

const PositionSensorsPage = () => {
  return (
      ...
      <div className="relative max-w-4xl mx-auto">
        ...
        <header className="mb-10 text-center">
          ...
          <p className="text-muted-foreground text-sm tracking-wide">
            Position Sensor HMI Components
          </p>
        </header>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {positionSensors.map((sensor) => (
            <Link
              key={sensor.tag}
              to={`/position-sensor/${sensor.tag}`}
              className={cn(
