...
const TempSensor4840Main = () => {
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({
    ...
    instrumentTag: savedConfig.TAGNAME || '1540-TI-4840',
    description: savedConfig.DESC || 'Pass 2 Catalyst In',
  ...
  useEffect(() => {
    const syncConfigFromContext = () => {
      ...
      setControllerData(prev => ({
        ...prev,
        instrumentTag: latestConfig.TAGNAME || '1540-TI-4840',
        description: latestConfig.DESC || 'Pass 2 Catalyst In',
        pvUnits: sanitizedEU,
  ...
  useEffect(() => {
    (window as any).updateTempSensor4840ControllerData = (newData: Partial<ControllerData>) => {
      setControllerData(prev => ({ ...prev, ...newData }));
    };
    (window as any).updateTempSensor4840SecondaryData = (newData: Partial<SecondaryControllerData>) => {
      setSecondaryData(prev => ({ ...prev, ...newData }));
    };
    
    return () => {
      delete (window as any).updateTempSensor4840ControllerData;
      delete (window as any).updateTempSensor4840SecondaryData;
    };
  }, []);
...
export default TempSensor4840Main;
