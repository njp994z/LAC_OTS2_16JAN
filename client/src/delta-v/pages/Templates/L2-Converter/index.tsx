...
import { defaultPositionL2Converter } from "./defalutPosition";

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
const LAYOUT_ID = "L2Converter";
...
const customColors = [
  ...
  { id: "transparent", label: "Transparent", fill: "transparent" },
];
...
const L2Converter = () => {
  ...
  const changeEdgeStrokeWidth = (id: number, delta: number) =>
    setEdges((prev) =>
      prev.map((e) => {
        if (e.id !== id) return e;
        const defaultW = e.style === "dashed" ? 3 : 6;
        const current = e.strokeWidth ?? defaultW;
        return { ...e, strokeWidth: Math.max(1, current + delta) };
      }),
    );
  ...
  const moveEdge = (id: number, dx: number, dy: number) =>
    setEdges((prev) =>
      prev.map((e) =>
        e.id === id
  ...
  const duplicateEdge = (id: number) => {
    const source = edges.find((e) => e.id === id);
    if (!source) return;
    const newId = edgeCounterRef.current++;
    const dupe: FlowEdge = {
      ...source,
      id: newId,
      x1: source.x1 + 20,
    ...
    setEdges((prev) => [...prev, dupe]);
    setSelectedEdge(newId);
  };

  // ── keyboard arrow-key movement for selected edge ────────────────────────
  useEffect(() => {
    const step = 5;
  ...
  useEffect(() => {
    ...
    if (!layoutData && !isLoading && !isUninitialized) {
      ...
      if (
        defaultPositionL2Converter.edges &&
        defaultPositionL2Converter.edges.length > 0
      ) {
        const maxId = Math.max(
          ...defaultPositionL2Converter.edges.map((e) => e.id),
        );
      ...
      return (
        <ContextMenu key={edge.id}>
          ...
          <ContextMenuContent>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={(e) =>
                      handleStopPropagation(e, () =>
                        changeEdgeStrokeWidth(edge.id, 1),
                      )
                    }
                  >
                    +
            ...
            <ContextMenuCheckboxItem
              checked={selectedEdge === edge.id}
              onCheckedChange={(checked) =>
                setSelectedEdge(checked ? edge.id : null)
              }
            >
              Select for Arrow Keys
            </ContextMenuCheckboxItem>
            <ContextMenuSub>
            ...
            <ContextMenuItem
              onClick={(e) =>
                handleStopPropagation(e, () => duplicateEdge(edge.id))
              }
            >
              Duplicate Edge
            </ContextMenuItem>
      ...
      <div className="w-full h-screen flex flex-col pt-10 relative">
        ...
        <div className="flex-1 overflow-auto bg-gray-50">
          ...
          >
            ...
            {/* Elements */}
            {L2ConverterElements.map((element, index) => {
              const elData = L2ConverterElements_map[element];
