...
const TEMP_SENSOR_7823_PID_CONFIG_KEY = 'temp_sensor_7823_pid_config';
...
const TempSensor7823Faceplate3A = () => {
  ...
  const [config, setConfig] = useState<PIDHxBypassConfig>({
    ...defaultPIDHxBypassConfig,
    loop_tag: '1540-TI-7823',
    service_desc: 'SH 1B Outlet Temp.',
    eng_units: 'F',
  });
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_7823_PID_CONFIG_KEY);
    if (saved) {
      try {
  ...
  const saveConfig = useCallback(() => {
    localStorage.setItem(TEMP_SENSOR_7823_PID_CONFIG_KEY, JSON.stringify(config));
...
export default TempSensor7823Faceplate3A;
