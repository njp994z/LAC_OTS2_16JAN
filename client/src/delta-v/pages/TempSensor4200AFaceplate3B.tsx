...
const TEMP_SENSOR_4200A_TIMESERIES_KEY = 'temp_sensor_4200a_timeseries';
const TEMP_SENSOR_4200A_PID_CONFIG_KEY = 'temp_sensor_4200a_pid_config';

const TempSensor4200AFaceplate3B = () => {
  const activeControllerId = '1540-TI-4200A';
  ...
  const [config, setConfig] = useState({
    ...
    loop_tag: '1540-TI-4200A',
    service_desc: 'Furnace Temp Out A',
    eng_units: 'F',
  });

  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4200A_PID_CONFIG_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4200A_TIMESERIES_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (logData.length > 0) {
      localStorage.setItem(TEMP_SENSOR_4200A_TIMESERIES_KEY, JSON.stringify(logData.slice(-maxRows)));
    }
  }, [logData, maxRows]);
  ...
  const handleClearLog = () => {
    setLogData([]);
    localStorage.removeItem(TEMP_SENSOR_4200A_TIMESERIES_KEY);
  };
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `temp_sensor_4200a_timeseries_${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.csv`;
    a.click();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 deltav-grid-pattern">
      <div className="container mx-auto py-6 px-4">
        ...
        >
          <ArrowLeft size={18} />
          <span>Back to 1540-TI-4200A Temperature Sensor</span>
        </Link>
...
export default TempSensor4200AFaceplate3B;
