...
const ALARM_INFO: Record<string, { name: string; priority: 'critical' | 'warning' }> = {
  HH: { name: 'High-High Temperature', priority: 'critical' },
  LL: { name: 'Low-Low Temperature', priority: 'critical' },
  H: { name: 'High Temperature', priority: 'warning' },
  L: { name: 'Low Temperature', priority: 'warning' },
...
const Faceplate3F = () => {
  ...
  // Monitor alarm state changes and log new alarms
  useEffect(() => {
    ...
    (Object.keys(ALARM_INFO) as Array<keyof typeof ALARM_INFO>).forEach((alarmType) => {
      ...
      // Alarm just triggered (false -> true)
      if (!wasActive && isActive) {
        ...
        toast({
          ...
          description: `PV: ${state.syncedPV.toFixed(1)}°F | Limit: ${limit}°F`,
                  ...
                  {state.alarmLog.length === 0 ? (
                    ...
                    state.alarmLog.map((alarm) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2">
                          <span className="text-foreground">{alarm.value.toFixed(1)}°F</span>
                          <span className="text-muted-foreground text-xs ml-1">(lim: {alarm.limit}°F)</span>
                        </td>
                        <td className="px-4 py-2">
                          <span
