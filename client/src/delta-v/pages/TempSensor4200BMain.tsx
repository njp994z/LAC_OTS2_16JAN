...
const TempSensor4200BMain = () => {
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({
    ...
    instrumentTag: savedConfig.TAGNAME || '1540-TI-4200B',
    description: savedConfig.DESC || 'Furnace Temp Out B',
  ...
  useEffect(() => {
    const syncConfigFromContext = () => {
      ...
      setControllerData(prev => ({
        ...prev,
        instrumentTag: latestConfig.TAGNAME || '1540-TI-4200B',
        description: latestConfig.DESC || 'Furnace Temp Out B',
        pvUnits: sanitizedEU,
  ...
  useEffect(() => {
    (window as any).updateTempSensor4200BControllerData = (newData: Partial<ControllerData>) => {
      setControllerData(prev => ({ ...prev, ...newData }));
    };
    (window as any).updateTempSensor4200BSecondaryData = (newData: Partial<SecondaryControllerData>) => {
      setSecondaryData(prev => ({ ...prev, ...newData }));
    };
    
    return () => {
      delete (window as any).updateTempSensor4200BControllerData;
      delete (window as any).updateTempSensor4200BSecondaryData;
    };
  }, []);
...
export default TempSensor4200BMain;
