...
// Independent localStorage keys for this controller
const TEMP_SENSOR_4200B_TIMESERIES_KEY = 'temp_sensor_4200b_timeseries';
const TEMP_SENSOR_4200B_PID_CONFIG_KEY = 'temp_sensor_4200b_pid_config';

const TempSensor4200BFaceplate3B = () => {
  const activeControllerId = '1540-TI-4200B';
  ...
  const [config, setConfig] = useState({
    ...
    loop_tag: '1540-TI-4200B',
    service_desc: 'Furnace Temp Out B',
    eng_units: 'F',
  });

  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4200B_PID_CONFIG_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4200B_TIMESERIES_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (logData.length > 0) {
      localStorage.setItem(TEMP_SENSOR_4200B_TIMESERIES_KEY, JSON.stringify(logData.slice(-maxRows)));
    }
  }, [logData, maxRows]);
  ...
  const handleClearLog = () => {
    setLogData([]);
    localStorage.removeItem(TEMP_SENSOR_4200B_TIMESERIES_KEY);
  };
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `temp_sensor_4200b_timeseries_${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.csv`;
    a.click();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 deltav-grid-pattern">
      <div className="container mx-auto py-6 px-4">
        <Link
          to="/settings/controller-outputs/faceplates/temp-sensor/1540-TI-4200B"
          className="inline-flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition-colors mb-4"
        >
          <ArrowLeft size={18} />
          <span>Back to 1540-TI-4200B Temperature Sensor</span>
        </Link>
...
export default TempSensor4200BFaceplate3B;