...
const JugValveDetail = () => {
  const activeControllerId = '1540-HCV-4282';
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({
    ...
    instrumentTag: savedConfig.TAGNAME || jugValve.name,
    description: savedConfig.DESC || jugValve.description,
    ...
    <div className="min-h-screen bg-background p-8">
      ...
      <div className="relative max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <Link href="/settings/controller-outputs/faceplates/valve-blocks/hand-control" className={cn("inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200", "text-muted-foreground hover:text-foreground", "hover:bg-card/50 active:scale-95")}>
            <ArrowLeft size={18} />
        ...
        <header className="mb-10 text-center">
          <h1 className={cn("text-3xl font-bold mb-2 tracking-tight", "bg-gradient-to-r from-primary via-cyan-400 to-primary bg-clip-text text-transparent")}>
            {jugValve.name} - {jugValve.description}
          </h1>
          <p className="text-muted-foreground text-sm tracking-wide">Hand Control Valve • Primary Faceplate</p>
        </header>
        <main className="flex flex-col items-center gap-10">
          <section className="flex flex-col items-center">
            <ValveFaceplate data={controllerData} onSelect={() => console.log('Faceplate selected:', controllerData.instrumentTag)} isTransparent={config.TRANSPARENT_BG} valveImageSrc={jugValveImage} />
          </section>
        </main>
      </div>
    </div>
