...
const TEMP_SENSOR_4828_TIMESERIES_KEY = 'temp_sensor_4828_timeseries';
const TEMP_SENSOR_4828_PID_CONFIG_KEY = 'temp_sensor_4828_pid_config';

const TempSensor4828Faceplate3B = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(TEMP_SENSOR_4828_TIMESERIES_KEY);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (logData.length > 0) {
      localStorage.setItem(TEMP_SENSOR_4828_TIMESERIES_KEY, JSON.stringify(logData.slice(-maxRows)));
    }
  }, [logData, maxRows]);
  ...
  const handleClearLog = () => {
    setLogData([]);
    localStorage.removeItem(TEMP_SENSOR_4828_TIMESERIES_KEY);
  };
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `temp_sensor_4828_timeseries_${new Date().toISOString().slice(0, 19).replace(/[:.]/g, '-')}.csv`;
    a.click();
...
export default TempSensor4828Faceplate3B;
