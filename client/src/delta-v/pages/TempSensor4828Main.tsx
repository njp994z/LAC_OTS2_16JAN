...
const TempSensor4828Main = () => {
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({
    ...
    instrumentTag: savedConfig.TAGNAME || '1540-TI-4828',
    description: savedConfig.DESC || 'Pass 1 Catalyst Temp. Bottom B',
  ...
  useEffect(() => {
    const syncConfigFromContext = () => {
      ...
      setControllerData(prev => ({
        ...prev,
        instrumentTag: latestConfig.TAGNAME || '1540-TI-4828',
        description: latestConfig.DESC || 'Pass 1 Catalyst Temp. Bottom B',
        pvUnits: sanitizedEU,
  ...
  useEffect(() => {
    (window as any).updateTempSensor4828ControllerData = (newData: Partial<ControllerData>) => {
      setControllerData(prev => ({ ...prev, ...newData }));
    };
    (window as any).updateTempSensor4828SecondaryData = (newData: Partial<SecondaryControllerData>) => {
      setSecondaryData(prev => ({ ...prev, ...newData }));
    };
    
    return () => {
      delete (window as any).updateTempSensor4828ControllerData;
      delete (window as any).updateTempSensor4828SecondaryData;
    };
  }, []);
...
export default TempSensor4828Main;
