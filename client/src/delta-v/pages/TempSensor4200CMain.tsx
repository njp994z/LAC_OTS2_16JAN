...
const TempSensor4200CMain = () => {
  ...
  const [controllerData, setControllerData] = useState<ControllerData>({
    ...
    instrumentTag: savedConfig.TAGNAME || '1540-TI-4200C',
    description: savedConfig.DESC || 'Furnace Temp Out C',
  ...
  useEffect(() => {
    const syncConfigFromContext = () => {
      ...
      setControllerData(prev => ({
        ...prev,
        instrumentTag: latestConfig.TAGNAME || '1540-TI-4200C',
        description: latestConfig.DESC || 'Furnace Temp Out C',
        pvUnits: sanitizedEU,
  ...
  useEffect(() => {
    (window as any).updateTempSensor4200CControllerData = (newData: Partial<ControllerData>) => {
      setControllerData(prev => ({ ...prev, ...newData }));
    };
    (window as any).updateTempSensor4200CSecondaryData = (newData: Partial<SecondaryControllerData>) => {
      setSecondaryData(prev => ({ ...prev, ...newData }));
    };
    
    return () => {
      delete (window as any).updateTempSensor4200CControllerData;
      delete (window as any).updateTempSensor4200CSecondaryData;
    };
  }, []);
      ...
      <div className="relative max-w-4xl mx-auto">
        ...
        <header className="mb-8 text-center">
          <h1 className={cn(
            ...
            1540-TI-4200C Furnace Temp Out C
          </h1>
...
export default TempSensor4200CMain;
