...
const pythonCode = `"""
Drying Tower Mass Balance Calculator
====================================
Core calculation logic for Sulfuric Acid Drying Tower simulation.
Performs static mass and energy balance calculations for the tower.

Reference: Industrial sulfuric acid plant drying tower operation
"""
...
def calculate_tower(inputs):
    ...
    Parameters:
        inputs (dict): Dictionary containing all required input values
            - x_H2SO4_AD0: Inlet acid H2SO4 mass fraction (0.93-0.98)
            - x_H2O_AD0: Inlet acid water mass fraction
            - Temp_AD0: Inlet acid temperature (°F)
            - Pressure_AD0: Inlet acid pressure (psig)
            - Flow_AD0: Inlet acid flow rate (gpm)
            - dP_BME: Tower pressure drop (in WC)
            - Packing_Depth_ft: Packing depth (ft)
    ...
    try:
        ...
        # More packing = better mass transfer = higher efficiency
        efficiency = min(0.90 + 0.01 * packing_depth, EFFICIENCY_BASE)
        
        # ── Inlet Acid Mass Flow Calculations ────────────────────────────────
        # Convert volumetric flow to mass flow
        # mass_flow = gpm * 60 min/hr * 8.34 lb/gal * SG
        ...
        # Calculate outlet mass fractions
        ...
        # Outlet flow increases slightly due to absorbed SO3
        flow_gpm_out = flow_gpm * (mass_out_lbhr / mass_in_lbhr) if mass_in_lbhr > 0 else 0
        
        # Pressure drop through tower (convert in WC to psig)
        pressure_out_psig = pressure_psig - (dp_inwc / 27.68)
        
        # ── Prepare results for GUI ──────────────────────────────────────────
        results.update({
            # Acid section outputs
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/95 backdrop-blur">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/unit-operation/drying-tower" data-testid="link-back">
