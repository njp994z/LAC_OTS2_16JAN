...
import sys
import json
from catalyst_database import get_catalyst, CatalystProperties, get_catalyst_list

# Constants
R_GAS = 10.7316             # ft3·psia/(lbmol·R)
ATM_TO_PSIA = 14.696
INWC_TO_PSI = 1 / 27.68
FT3_PER_L = 0.0353146667

# Reaction: SO2 + 0.5 O2 -> SO3   -> delta_nu = -0.5
DELTA_NU = -0.5

# Approximate Cp (J/mol·K) - average
CP = {"SO2": 45.0, "O2": 36.0, "SO3": 60.0, "N2": 35.0}

# delta_H_rxn (J/mol SO2 converted) - exothermic
DH_RXN = -99000.0
...
def reaction_rate_eklund(T_C: float, X: float, P_atm: float, y0: dict, activity: float) -> float:
    ...
    Kp = Kp_eklund(T_C)
    rate = k * (pSO2 - pSO3 / (Kp * math.sqrt(max(pO2, 1e-10)))) * activity
    return max(0.0, rate)
...
def simulate_pass(
    ...
    barometric_psia: float = 14.3
) -> dict:
    # Calculate N2 if not provided
    if inlet_n2_pct is None:
        inlet_n2_pct = 100.0 - inlet_so2_pct - inlet_o2_pct - inlet_so3_pct
    
    cat: CatalystProperties = get_catalyst(catalyst_name)
    activity = cat.activity_fresh

    # Inlet conditions
    P_in_psia = barometric_psia + inlet_P_inwc * INWC_TO_PSI
    ...
    total_lbmol_hr = inlet_total_scfm / 379.5   # approx standard ft3/lbmol
    ...
    n_points = 5
    z_points = [bed_depth_total_ft * i / (n_points - 1) for i in range(n_points)]
    dz_ft = z_points[1] - z_points[0] if n_points > 1 else 0

    # Catalyst mass per segment (assume uniform)
    mass_total_kg = catalyst_volume_liters * cat.bulk_density_kg_m3 / 1000
    W_seg_kg = mass_total_kg / (n_points - 1) if n_points > 1 else mass_total_kg

    # Results arrays
    T_C_arr   = [0.0] * n_points
    X_arr     = [0.0] * n_points
    P_inwc_arr = [0.0] * n_points
    v_fts_arr = [0.0] * n_points
    so2_pct   = [0.0] * n_points
    o2_pct    = [0.0] * n_points
    ...
    for i in range(1, n_points):
        ...
        P_psia -= dP_psi
        P_inwc_arr[i] = (P_psia - barometric_psia) / INWC_TO_PSI
    ...
    y_out = mole_fractions_at_X(y0, X)
    total_out_scfm = inlet_total_scfm * (1 + DELTA_NU * y0["SO2"] * X)
    so2_out_scfm = total_out_scfm * y_out["SO2"]
    so3_out_scfm = total_out_scfm * y_out["SO3"]
    o2_out_scfm  = total_out_scfm * y_out["O2"]
    n2_out_scfm  = total_out_scfm * y_out["N2"]

    results = {
        "bed_depths_ft": [round(z, 2) for z in z_points],
        "temps_C": [round(t, 1) for t in T_C_arr],
        "temps_F": [round(t * 9/5 + 32, 1) for t in T_C_arr],
        "overall_conv_pct": [round(x, 2) for x in X_arr],
        "bed_conv_pct": [round(X_arr[i] - X_arr[i-1] if i>0 else 0.0, 2) for i in range(n_points)],
        "pressures_inwc": [round(p, 2) for p in P_inwc_arr],
        "velocities_fts": [round(v, 2) for v in v_fts_arr],
        "so2_pct": [round(s, 3) for s in so2_pct],
        "o2_pct": [round(o, 3) for o in o2_pct],
        "bed_depth_total_ft": round(bed_depth_total_ft, 2),
        "catalyst_mass_kg": round(mass_total_kg, 0),
        ...
        "inlet": {
            ...
            "TEMPERATURE_C": round(inlet_T_C, 1),
        ...
        "outlet": {
            ...
            "TEMPERATURE_C": round(T_C_arr[-1], 1),
            "TEMPERATURE_F": round(T_C_arr[-1] * 9/5 + 32, 0)
        },
...
def main():
    ...
    try:
        ...
        # Extract parameters
        inlet_T_C = float(input_data.get("inlet_T_C", 390))
        inlet_P_inwc = float(input_data.get("inlet_P_inwc", 150))
        ...
        inlet_total_scfm = float(input_data.get("inlet_total_scfm", 150000))
        diameter_ft = float(input_data.get("diameter_ft", 42.0))
        catalyst_volume_liters = float(input_data.get("catalyst_volume_liters", 89200))
        ...
        results = simulate_pass(
            ...
            inlet_so2_pct=inlet_so2_pct,
            inlet_o2_pct=inlet_o2_pct,
            inlet_so3_pct=inlet_so3_pct,
            inlet_total_scfm=inlet_total_scfm,
            diameter_ft=diameter_ft,
            catalyst_volume_liters=catalyst_volume_liters,
        ...
        print(json.dumps({
            "success": True,
            "results": results,
            "catalyst_list": get_catalyst_list()
        }))
